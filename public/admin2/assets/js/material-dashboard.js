...
function applyStoredSettings() {
    ...
    // 3. Terapkan Tipe Sidenav (Dark/Transparent/White)
    if (storedSidenavType) {
        const typeButton = document.querySelector(
            `.fixed-plugin .btn[data-class="${storedSidenavType}"]`
        );
        if (typeButton) typeButton.click();
    }

    // 4. Terapkan Navbar Fixed
    const navbarFixedSwitch = document.getElementById("navbarFixed");
    if (navbarFixedSwitch) {
        if (storedNavbarFixed === "true" && !navbarFixedSwitch.checked) {
            navbarFixedSwitch.click();
        } else if (storedNavbarFixed === "false" && navbarFixedSwitch.checked) {
            navbarFixedSwitch.click();
        }
    }
    ...
    const darkModeSwitch = document.querySelector(
        '.fixed-plugin input[type="checkbox"][onclick="darkMode(this)"]'
    );
    if (darkModeSwitch) {
        if (storedDarkMode === "true" && !darkModeSwitch.checked) {
            darkModeSwitch.click();
        } else if (storedDarkMode === "false" && darkModeSwitch.checked) {
            darkModeSwitch.click();
        }
    }
}
...
function sidebarType(a) {
    ...
    localStorage.setItem("sidenavType", color);
...
function navbarFixed(el) {
    ...
    localStorage.setItem("navbarFixed", !el.getAttribute("checked"));
...
function darkMode(el) {
    ...
    localStorage.setItem("darkMode", !el.getAttribute("checked"));
